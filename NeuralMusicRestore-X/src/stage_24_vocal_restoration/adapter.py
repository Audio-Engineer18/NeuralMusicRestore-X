...
--------------------------
Uses SGMSE (https://github.com/sp-uhh/sgmse) for conditional vocal restoration.
...
# Check if SGMSE is available
try:
    from sgmse import SGMSEModel
    from sgmse.utils import load_pretrained_model
except ImportError:
    raise ImportError(
        "SGMSE not found. Install it from: https://github.com/sp-uhh/sgmse"
    )


class VocalRestorationAdapter:
    """Python adapter for vocal restoration using SGMSE."""
    ...
    def _load_model(self, model_path: Optional[str] = None):
        """Load the SGMSE model."""
        if model_path is None:
            model_path = os.path.expanduser("~/.cache/sgmse/model.pth")
    ...
    def process(
        ...
        """Apply conditional vocal restoration to the input audio.
        ...
        Returns:
            Restored vocals as a numpy float32 array.
        """
